...
# ЗАМЕНА: Модель Вейбулла (Weibull / Stretched Exponential)
# Идеальна для асимметричного роста: резкий старт -> плавное плато
def weibull_ef(x, A, B, C):
    # A = Plateau (Максимум)
    # B = Scale (Масштаб по X, аналог постоянной времени)
    # C = Shape (Форма). 
    #     C = 1.0 -> Обычная экспонента (резкий старт).
    #     C > 1.0 -> S-образная кривая.
    #     C < 1.0 -> Очень резкий взлет.
    x_safe = np.maximum(x, 1e-6)
    return A * (1.0 - np.exp(-np.power(x_safe / B, C)))
...
def perform_fit(use_all=True):
    ...
    # Weibull Linoleic
    try:
        # p0: A=2.0, B=50, C=1.0 (начинаем с чистой экспоненты)
        # bounds: C от 0.5 до 3.0 (чтобы не было "ступеньки")
        pL, _ = curve_fit(weibull_ef, L_sn123_all[idx_L], EF_L_all[idx_L], 
                          p0=[2.0, 50.0, 1.0], 
                          bounds=([0.1, 1.0, 0.5], [5.0, 300.0, 3.0])) 
    except: 
        pL = [2.0, 50.0, 1.0]
...
# Используем weibull_ef
line_L_abs, = ax_L_abs.plot(x_range, x_range * weibull_ef(x_range, *best_h), 'r-')
...
# Используем weibull_ef
line_L_ef, = ax_L_ef.plot(x_range, weibull_ef(x_range, *best_h), 'r-')
...
# --- 5. Виджеты ---
# Слайдеры для Weibull
s_AL = Slider(plt.axes([0.15, 0.30, 0.3, 0.015]), 'Weibull A (Max)', 0.1, 5.0, valinit=best_h[0], valstep=0.01)
s_BL = Slider(plt.axes([0.15, 0.28, 0.3, 0.015]), 'Weibull B (Scale)', 1, 200, valinit=best_h[1], valstep=0.01)
# C=1.0 - это резкий старт. C>1 - S-образность.
s_CL = Slider(plt.axes([0.15, 0.26, 0.3, 0.015]), 'Weibull C (Shape)', 0.5, 3.0, valinit=best_h[2], valstep=0.01)

s_AL_MM = Slider(plt.axes([0.15, 0.22, 0.3, 0.015]), 'Lin MM A', 0.1, 10, valinit=best_mm[0], color='green', valstep=0.01)
s_BL_MM = Slider(plt.axes([0.15, 0.20, 0.3, 0.015]), 'Lin MM B', 0.1, 200, valinit=best_mm[1], color='green', valstep=0.01)
s_AO = Slider(plt.axes([0.60, 0.28, 0.3, 0.015]), 'Ole MM A', 10, 500, valinit=best_o[0], color='salmon', valstep=0.01)
s_BO = Slider(plt.axes([0.60, 0.26, 0.3, 0.015]), 'Ole MM B', 1, 300, valinit=best_o[1], color='salmon', valstep=0.01)
...
def update(val):
    line_L_ef.set_ydata(weibull_ef(x_range, s_AL.val, s_BL.val, s_CL.val))
    line_L_abs.set_ydata(x_range * weibull_ef(x_range, s_AL.val, s_BL.val, s_CL.val))
