...
# ЗАМЕНА: Уравнение Хилла (выходит на плато A)
def smooth_hill_ef(x, A, B, C):
    # A = Plateau (Vmax)
    # B = K_half (значение x, где y = A/2)
    # C = Hill coefficient (степень кривизны)
    x_safe = np.maximum(x, 1e-6)
    # Формула: A * x^C / (B^C + x^C)
    return (A * np.power(x_safe, C)) / (np.power(B, C) + np.power(x_safe, C))
...
def perform_fit(use_all=True):
    ...
    # Hill Linoleic (Вместо Haldane)
    try:
        # p0: A=2.0 (плато), B=30 (полунасыщение), C=2.0 (степень)
        # bounds: A[0.1-5], B[1-200], C[0.1-10]
        pL, _ = curve_fit(smooth_hill_ef, L_sn123_all[idx_L], EF_L_all[idx_L], 
                          p0=[2.0, 30.0, 2.0], bounds=([0.1, 1.0, 0.1], [5.0, 200.0, 10.0]))
    except: pL = [2.0, 30.0, 2.0]
...
# Используем smooth_hill_ef
line_L_abs, = ax_L_abs.plot(x_range, x_range * smooth_hill_ef(x_range, *best_h), 'r-')
...
# Используем smooth_hill_ef
line_L_ef, = ax_L_ef.plot(x_range, smooth_hill_ef(x_range, *best_h), 'r-')
...
# --- 5. Виджеты ---
# Обновлены диапазоны для Hill Equation
s_AL = Slider(plt.axes([0.15, 0.30, 0.3, 0.015]), 'Hill A (Max)', 0.1, 5.0, valinit=best_h[0], valstep=0.01)
s_BL = Slider(plt.axes([0.15, 0.28, 0.3, 0.015]), 'Hill B (K)', 1, 200, valinit=best_h[1], valstep=0.01)
s_CL = Slider(plt.axes([0.15, 0.26, 0.3, 0.015]), 'Hill C (n)', 0.1, 10.0, valinit=best_h[2], valstep=0.01)
...
def update(val):
    # Обновлено на smooth_hill_ef
    line_L_ef.set_ydata(smooth_hill_ef(x_range, s_AL.val, s_BL.val, s_CL.val))
    line_L_abs.set_ydata(x_range * smooth_hill_ef(x_range, s_AL.val, s_BL.val, s_CL.val))
