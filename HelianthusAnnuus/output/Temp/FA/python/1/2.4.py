...
# --- Модель для Линолевой (Haldane: Рост -> Пик -> Спад) ---
def haldane_ef(x, A, B, C):
    """
    EF = (A * x) / (B + x + x^2/C)
    При малых x растет (как A/B * x), потом пик, потом падает к 0 (или к константе, если модифицировать).
    """
    denom = B + x + (x**2 / C)
    return (A * x) / denom

def haldane_abs(x, A, B, C):
    """ sn-2 = x * EF """
    return x * haldane_ef(x, A, B, C)

# --- Модель для Олеиновой (Michaelis EF: Спад) ---
...
# Линолевая (Haldane)
# Начальные прикидки: A=Высота, B=Старт, C=Ширина пика
try:
    popt_L, _ = curve_fit(haldane_ef, L_sn123_all[valid_L], EF_L_all[valid_L], 
                          p0=[100, 20, 50], bounds=(0.1, 1000))
    best_A_L, best_B_L, best_C_L = popt_L
except:
    best_A_L, best_B_L, best_C_L = 100, 20, 50 # Fallback
...
line_L_abs, = ax_L_abs.plot(x_range, haldane_abs(x_range, best_A_L, best_B_L, best_C_L), c='red', lw=2)
ax_L_abs.set_title('Linoleic: sn-2 (Haldane Model)')
...
line_L_ef, = ax_L_ef.plot(x_range, haldane_ef(x_range, best_A_L, best_B_L, best_C_L), c='red', lw=2, linestyle='--')
ax_L_ef.set_title('Linoleic: EF (Peak & Drop)')
...
# Oleic Sliders (Michaelis)
ax_AO = plt.axes([0.60, 0.20, 0.3, 0.03], facecolor=axcolor)
ax_BO = plt.axes([0.60, 0.16, 0.3, 0.03], facecolor=axcolor)

# Слайдеры L
s_AL = Slider(ax_AL, 'Lin A (Height)', 10.0, 300.0, valinit=best_A_L, color='lightblue')
s_BL = Slider(ax_BL, 'Lin B (Start)', 1.0, 100.0, valinit=best_B_L, color='lightblue')
s_CL = Slider(ax_CL, 'Lin C (Decay)', 1.0, 200.0, valinit=best_C_L, color='lightblue')
...
def update(val):
    ...
    # Update Lines
    line_L_abs.set_ydata(haldane_abs(x_range, al, bl, cl))
    line_L_ef.set_ydata(haldane_ef(x_range, al, bl, cl))
    
    line_O_abs.set_ydata(michaelis_abs(x_range, ao, bo))
    line_O_ef.set_ydata(michaelis_ef(x_range, ao, bo))
    
    # Calc MAE (Absolute values)
    pred_L = haldane_abs(L_sn123_all, al, bl, cl)
